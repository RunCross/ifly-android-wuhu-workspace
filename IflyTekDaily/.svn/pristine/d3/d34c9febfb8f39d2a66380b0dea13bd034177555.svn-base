...
public class DailyWriteActivity extends Activity {
	...
	private void initText() {
		...
		if (todayStr.equals(dateStr)) {
			if (!isWriteTodayDaily()) {
				Toast.makeText(DailyWriteActivity.this, "你现在填写为昨日日报", 200)
						.show();
			}
		}
	...
	private void writeTodayDaily() {
		...
		try {
			...
			if ("0".equals(map.get("StateCode"))) {
				...
				DailyWriteActivity.this.finish();
				if (!isWriteTodayDaily()) {
					Toast.makeText(DailyWriteActivity.this, "填写昨日日报成功！", 200)
							.show();
				} else {
					Toast.makeText(DailyWriteActivity.this, "填写成功！", 200)
							.show();
				}
			} else {
				Toast.makeText(DailyWriteActivity.this, "提交失败！", 200).show();
	...
	/**
	 * 重写onKeyDown方法，判断按下返回键，退出Activity
	 */
	@Override
