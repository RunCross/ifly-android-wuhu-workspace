...
public class AutoSendServices extends Service {
	...
	/**
	 * 当重新进入服务时 就是想要自动填写日报 当今天的日报已填写 就不再自动发送 如果今天的日报为空 就自动发送设定的日报
	 * 
	 * @return
	 */
	@Override
	public int onStartCommand(Intent intent, int flags, int startId) {
		System.out.println("服务被调用"+falg);
		if (falg) {
			falg = false;
		} else {
			if (judgeToday()) {
				autoSend();
				// notifyUser("正在发送","调用发送日报函数");
			} else {
	...
	private void autoSend() {
		...
		if ("".endsWith(dailyText) && "".endsWith(cooditeText)) {
			// 提示日报内容为空 发送失败
			notifyUser("您设置的日报内容为空", "日报内容为空，发送不成功，请您手动填写！");
		} else {
