...
public class GetAreaInfo {
	...
	//地图url
	private static final String URL_AREA_INFO = Info.LoginServer + "/connect/app/exploration/area?cyt=1";
				...
				for (int i = 0; i < list.getLength(); i++) {
					...
					do{
						if(f.getNodeName().equals("id")){
							...
							floorInfo.prog_area = f.getFirstChild().getNodeValue();
						}else if(f.getNodeName().equals("area_type")){
							floorInfo.area_type = f.getFirstChild().getNodeValue();
					...
					if(Info.whatMap != 0){
						...
						if(isClear){
							...
							if(!floorInfo.area_type.equals(Info.runFactor) && !floorInfo.prog_area.equals("100")){
								floorInfos.add(floorInfo);
							}
						}
					}
