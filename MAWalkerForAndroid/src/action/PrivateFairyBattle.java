...
public class PrivateFairyBattle {
	...
	public static boolean run() throws Exception {
		...
		try {
			...
			if (Boolean.valueOf(xpath.evaluate("count(//private_fairy_top) > 0", doc, XPathConstants.BOOLEAN).toString())) {
			...
			// 检查觉醒
			if (Boolean.valueOf(xpath.evaluate("count(//ex_fairy/rare_fairy)>0", doc, XPathConstants.BOOLEAN).toString())) {
				// Yes
