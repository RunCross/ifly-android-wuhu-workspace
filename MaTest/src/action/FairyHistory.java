...
public class FairyHistory {
	//获取妖精列表
	private static final String URL_FAIRY_HISTORY = Info.LoginServer + "/connect/app/exploration/fairyhistory?cyt=1";
	...
	private static boolean parse(Document doc,FairyInfo fairyInfo) throws Exception {
		try {
			...
			NodeList list = (NodeList)xpath.evaluate("//fairy_history/fairy/attacker_history/attacker[user_id="+Process.info.userId+"]", doc, XPathConstants.NODESET);
			if(list.getLength() > 0){
				return false;
			}else{
				int currentHp = Integer.parseInt(xpath.evaluate("//fairy_history/fairy/hp", doc));
				int maxHp = Integer.parseInt(xpath.evaluate("//fairy_history/fairy/hp_max", doc));
